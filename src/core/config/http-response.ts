...
export function succeed<D>({ data, code = HttpStatus.OK, message }: SuccessArg<D>): Success<D> {
  return { code, message, data, success: true };
}

export function fail({ error, code = HttpStatus.INTERNAL_SERVER_ERROR, message }: FailureArg) {
